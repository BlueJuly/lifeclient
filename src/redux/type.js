export const LOGIN = 'login';
export const LOGINSUCCESS = 'loginsuccess';
export const LOGINFAILED = 'loginfailed';
export const UPDATE_DEVICE_INFO = 'update_device_info';
export const FETCHING_TILES = 'fetching_tiles';
export const FETCH_TILES_SUCCESS = 'fetch_tiles_success';
export const FETCH_TILES_FAILED = 'fetch_tiles_failed';
export const FETCHING_CONTACTS = 'fetching_contacts';
export const FETCH_CONTACTS_SUCCESS = 'fetch_contacts_success';
export const FETCH_CONTACTS_FAILED = 'fetch_contacts_failed';
export const FETCHING_CARETEAMMEMBERS = 'fetching_careteamMembers';
export const FETCH_CARETEAMMEMBERS_SUCCESS = 'fetch_careteamMembers_success';
export const FETCH_CARETEAMMEMBERS_FAILED = 'fetch_careteamMembers_failed';
export const UPDATING_WEBRTCREDUCER = 'updating_webrtcreducer';
export const UPDATE_WEBRTC_ISFRONT = 'updating_webrtc_isfront';
export const UPDATE_WEBRTC_REMOTELIST = 'updating_webrtc_remotelist';
export const UPDATE_WEBRTC_REMOTESOCKETID = 'updating_webrtc_remotesocketid';
export const UPDATE_WEBRTC_REMOTEVIEWSRC = 'updating_webrtc_remoteviewsrc';
export const UPDATE_WEBRTC_SELFVIEWSRC = 'updating_webrtc_selfviewsrc';
export const UPDATE_WEBRTC_LOCALSTREAM = 'updating_webrtc_localstream';
export const UPDATE_WEBRTC_FAILED = 'updating_webrtc_failed';
export const RESET_WEBRTCREDUCER = 'reset_webrtcreducer';
export const UPDATE_WEBRTC_CALLSTATUS = 'updating_webrtc_callstatus';
